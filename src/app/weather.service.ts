...
export class WeatherService {
  ...
  getMinTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    let min: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=952d6b1a52fe15a7b901720074680562`).subscribe((weather: any) => {
  ...
  getForcaste(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<Array<any>> {
